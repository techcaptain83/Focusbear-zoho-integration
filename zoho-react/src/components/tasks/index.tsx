...
const ProjectTasks: React.FC = () => {
    ...
    const { portalId, projectId } = useParams();
    ...
    const handleGetProjectTasks = async () => {
        try {
            const result: any = await axios.get(
                `${process.env.REACT_APP_BACKEND_API}/portals/${portalId}/projects/${projectId}/tasks`,
                {
                    headers: {
    ...
    return (
        <div className="flex flex-col items-center justify-center">
            ...
            <div className='flex items-center justify-between w-full max-w-2xl mt-8'>
                <h1 className="text-2xl">Projects Tasks of: {profile?.firstName || ""} {profile?.lastName || ''}</h1>
                        ...
                        <div className="relative overflow-auto rounded-xl">
                            <div className="my-8 overflow-auto shadow-sm">
                                <table className="w-full text-sm border-collapse table-fixed">
                                    <thead>
                                        <tr>
                                            <th rowSpan={2} className="p-4 pt-0 pb-3 pl-8 font-medium text-center border-b dark:border-slate-600 text-slate-400 dark:text-slate-200 w-[50px]">S.N</th>
                                            <th rowSpan={2} className="p-4 pt-0 pb-3 pl-8 font-medium text-center border-b dark:border-slate-600 text-slate-400 dark:text-slate-200 w-[300px]">Name</th>
                                            <th rowSpan={2} className="p-4 pt-0 pb-3 font-medium text-center border-b dark:border-slate-600 text-slate-400 dark:text-slate-200 w-[150px]">Created Person</th>
                                            <th rowSpan={2} className="p-4 pt-0 pb-3 font-medium text-center border-b dark:border-slate-600 text-slate-400 dark:text-slate-200 w-[150px]">Priority</th>
                                            ...
                                            <th rowSpan={2} className="p-4 pt-0 pb-3 pr-8 font-medium text-center border-b dark:border-slate-600 text-slate-400 dark:text-slate-200 w-[200px]">Actions</th>
                                        </tr>
                                        <tr>
                                    ...
                                    <tbody className="bg-white dark:bg-slate-800">
                                        {
                                            tasks && tasks?.map((task: any, index: number) => (
                                                <tr key={index}>
                                                    ...
                                                    <td className="p-4 pl-8 border-b border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-400">{task?.name}</td>
                                                    <td className="p-4 text-center border-b border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-400">{task?.created_person}</td>
                                                    <td className="p-4 pr-8 text-center border-b border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-400">{task?.priority}</td>
            ...
            {
                showModal &&
                <TaskTimelogModal portalId={portalId!} projectId={projectId!} showModal={showModal} setShowModal={setShowModal} taskData={taskData} />
            }
        </div>
    )
}
