...
const TaskTimelogModal = ({ showModal, setShowModal, taskData, portalId, projectId }: ITaskTimeLogProps) => {
    ...
    const handleAddTimeLog = async (e:any) => {
        e.preventDefault()
        const fields = ["date", "bill_status", "hours", "notes"];
        let customError: any = {};
        ...
        const { date, bill_status, hours, notes } = formData;
        const body = {
            // owner: '39606834',
            date, 
            bill_status, 
        ...
        try {
            const result: any = await axios.post(
                `${process.env.REACT_APP_BACKEND_API}/portals/${portalId}/projects/${projectId}/tasks/${taskData?.id_string}/logs`,
                body,
                {
                    headers: {
            ...
            if(result){
                ...
                toast.success('Task Timelog added succesfully!')
                setTimeout(() => {
                    setFormData({
                        date: "",
                        bill_status: "",
